...
export function Game() {
    ...
    // async function handleBoardClick(cell: CellProps) {
    ...
    async function handleNumberPress(newValue: string) {
        ...
        lastCellChanged.push(selectedCell);
        console.log(lastCellChanged[lastCellChanged.length - 1].index)
        ...
        try {
            ...
            setBoard(board.map(cell => {
                if (cell.index === id) {
                    console.log("Certo? -", selectedCell.right)
                    if (!selectedCell.right) return { ...cell, color: 'red', value: newValue }
    ...
    async function handleRules(newValue: string) {
        ...
        console.log(selectedCellRow, "/", selectedCellColumn, "/", selectedCellBlock, "/", newValue)

        board.map(cell => {
            if(cell.index !== selectedCell.index) {
                // Verifica as células da mesma linha
                if(cell.row === selectedCellRow)
                    if(cell.value === newValue)
                        console.log(cell.index, "/", cell.value)
                        selectedCell.right = false

                // Verifica as células da mesma coluna
                if(cell.column === selectedCellColumn)
                    if(cell.value === newValue)
                        selectedCell.right = false

                // Verifica as células do mesmo bloco
                if(cell.block === selectedCellBlock)
                    if(cell.value === newValue)
                        selectedCell.right = false
            } else return cell;
        });
        ...
        //             if (cell.value === newValue) {
        //                 console.log("Deveria ter virado false")
        //                 setSelectedCell({ ...selectedCell, right: false })
        //             }
        //         }
        //     }
        // });
        console.log("Right da célula selecionada", selectedCell.right)
    }
                    ...
                    <Board
                        ...
                        renderItem={({ item }) => (
                            <Cell
                                ...
                                //color={item.right ? item.color : 'red'}
                                color={item.color}
